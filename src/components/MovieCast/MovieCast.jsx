...
export default function MovieCast({ results }) {

    let { responseMovieCrew, baseImg } = results

    return (
        <div className=' py-12'>
            <h2 className='capitalize text-white font-madimi text-center mb-4'>movie cast</h2>
            <Swiper
                className='movie-cast relative mb-14'
                spaceBetween={20}
                slidesPerView={2.5}
                freeMode={true}
                grabCursor={true}
            ...
            >
                ...
                {responseMovieCrew?.cast.map((actor, i) =>
                    actor['profile_path'] ?
                        <SwiperSlide key={i}>
                            <div>
            ...
            <h2 className='capitalize text-white font-madimi text-center mb-4 border-t-2 border-[#1E293B] pt-4'>movie crew</h2>
            <Swiper
                className='movie-crew relative'
                spaceBetween={20}
                slidesPerView={2.5}
                freeMode={true}
                grabCursor={true}
            ...
            >
                ...
                {responseMovieCrew?.crew.map((crew, i) =>
                    crew['profile_path'] ?
                        <SwiperSlide key={i}>
                            <div>
                                <Link className='' to={`/person-details/${crew.id}`}>
